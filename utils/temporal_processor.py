...
class TemporalProcessor:
    ...
    def preprocess(self, text: str, reference_time: Optional[datetime] = None) -> Dict[str, Any]:
        """
        ...
        if time_parts['date_part'] or time_parts['time_part']:
            ...
            if parsed_result:
                ...
                result['confidence'] = 0.8 if parsed_result.get('reminder_time') else 0.7
    ...
    def _extract_timezone(self, text: str) -> Optional[Dict[str, str]]:
        ...
        if tz_match:
            return {
                ...
                'cleaned_text': re.sub(tz_pattern, '', text, flags=re.IGNORECASE).strip()
            }
        ...
        for city, tz in self.city_to_tz.items():
            ...
            if re.search(city_pattern, text, re.IGNORECASE):
                return {
                    'timezone': tz,
                    'cleaned_text': re.sub(city_pattern, '', text, flags=re.IGNORECASE).strip()
                }
    ...
    def _extract_time_parts(self, text: str) -> Dict[str, Optional[str]]:
        ...
        for pattern in reminder_patterns:
            match = re.search(pattern, text, re.IGNORECASE)
            if match:
                result['reminder_part'] = match.group(1) if match.groups() else match.group(0)
                break
    ...
    def _parse_with_dateparser(
        ...
        datetime_str = ' '.join(filter(None, [time_parts['date_part'], time_parts['time_part']]))
        if datetime_str:
        ...
        if time_parts['reminder_part']:
            ...
            before_match = re.search(r'(\d+)\s*minutes?\s*before', time_parts['reminder_part'])
            if before_match and 'due_time' in result:
                ...
                reminder_dt = dateparser.parse(time_parts['reminder_part'], settings=settings)
                if reminder_dt:
                    result['reminder_time'] = reminder_dt.strftime('%H:%M')
                    result['reminder_date'] = result.get('due_date', reminder_dt.strftime('%Y-%m-%d'))
    ...
    def _handle_end_of_hour(self, text: str, reference_time: datetime) -> Dict[str, Any]:
        ...
        return {
            'processed_text': re.sub(r'end of (?:the )?hour', f"at {next_hour.strftime('%H:59')}", text, flags=re.IGNORECASE),
            'temporal_data': {
    ...
    def _handle_top_of_hour(self, text: str, reference_time: datetime) -> Dict[str, Any]:
        ...
        return {
            'processed_text': re.sub(r'top of (?:the )?hour', f"at {next_hour.strftime('%H:00')}", text, flags=re.IGNORECASE),
            'temporal_data': {
                'due_date': next_hour.strftime('%Y-%m-%d'),
                'due_time': next_hour.strftime('%H:%M'),
                'reminder_date': next_hour.strftime('%Y-%m-%d'),
    ...
    def _handle_end_of_day(self, text: str, reference_time: datetime) -> Dict[str, Any]:
        ...
        return {
            'processed_text': re.sub(r'end of (?:the )?day', "at 23:59", text, flags=re.IGNORECASE),
            'temporal_data': {
    ...
    def _handle_end_of_tonight(self, text: str, reference_time: datetime) -> Dict[str, Any]:
    ...
    def _handle_weekend(self, text: str, reference_time: datetime) -> Dict[str, Any]:
        ...
        return {
            'processed_text': re.sub(r'(?:this )?weekend', f"on {next_weekend.strftime('%Y-%m-%d')}", text, flags=re.IGNORECASE),
            'temporal_data': {
