...
from config.timezone_config import get_user_timezone, USER_TIMEZONES
from utils.timezone_converter import convert_time_between_users, process_task_with_timezones
...
def test_time_conversion():
    ...
    date_str, time_str = convert_time_between_users(
        base_date, "15:00", "Colin", "Joel"
    )
...
def test_task_processing():
    ...
    # Check conversions
    assert processed["due_time"] == "18:00", f"Expected due_time 18:00, got {processed['due_time']}"
    assert processed["reminder_time"] == "17:00", f"Expected reminder_time 17:00, got {processed['reminder_time']}"
    assert "timezone_info" in processed
    assert processed["timezone_info"]["converted"] == True
