...
from scripts.assistants_api_runner import get_or_create_assistant, parse_task, format_task_for_confirmation
...
for i, example in enumerate(examples, 1):
    ...
    try:
        ...
        if parsed.get('reminder_time'):
            print(f"Reminder: {parsed.get('reminder_date')} at {parsed.get('reminder_time')}")
        
        tz_info = parsed.get('timezone_info', {})
        if tz_info.get('converted'):
            print(f"\nTimezone conversion applied:")
            print(f"  From: Colin (PDT) → {assignee} ({get_timezone_abbreviation(assignee)})")
