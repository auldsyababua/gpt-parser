...
FEW_SHOT_EXAMPLES_FILE = os.path.join(os.path.dirname(__file__), '..', 'prompts', 'few_shot_examples.txt')
...
def handle_api_error(response, context=""):
    ...
    error_message = f"Error: API request failed for {context} with status {response.status_code}."
    try:
...
def api_request(method, url, **kwargs):
    ...
    log_interaction(f"Request: {method.upper()} {url}\nHeaders: {kwargs.get('headers')}\nPayload: {kwargs.get('json')}")
...
def get_or_create_assistant():
    ...
    try:
        with open(FEW_SHOT_EXAMPLES_FILE, "rb") as f:
            files = {'file': (os.path.basename(FEW_SHOT_EXAMPLES_FILE), f, 'application/octet-stream')}
            params = {'purpose': 'assistants'}
            upload_headers = {k: v for k, v in HEADERS.items() if k != "Content-Type"}
            response = api_request("post", files_url, headers=upload_headers, files=files, data=params)
    ...
    vs_response = api_request("post", vector_stores_url, headers=HEADERS, json=vs_payload)
    ...
    vs_file_response = api_request("post", vs_files_url, headers=HEADERS, json=vs_file_payload)
    ...
    response = api_request("get", assistants_url, headers=HEADERS, params={"limit": 100})
    if response.status_code == 200:
        ...
        for assistant in assistants:
            if assistant.get("name") == ASSISTANT_NAME:
                ...
                update_payload = {"tool_resources": {"file_search": {"vector_store_ids": [vector_store_id]}}}
                update_response = api_request("post", update_url, headers=HEADERS, json=update_payload)
...
def parse_and_send(assistant, input_text):
    ...
    vector_store_id = assistant.get("tool_resources", {}).get("file_search", {}).get("vector_store_ids", [None])[0]
    ...
    try:
        ...
        thread_response = api_request("post", threads_url, headers=HEADERS, json=thread_payload)
        ...
        log_interaction(f"Request: POST {GOOGLE_APPS_SCRIPT_URL}\nPayload: {json.dumps(parsed_json, indent=2)}")
        response = requests.post(GOOGLE_APPS_SCRIPT_URL, json=parsed_json, headers=gs_headers)
        log_interaction(f"Response: {response.status_code}\nBody: {response.text}")
        response.raise_for_status()
        print(f"Successfully sent data. Response from Google Apps Script ({response.status_code}):")
        print(response.text)
